...
- **Improve Vault Fault Tolerance:** currently Vault is pinned to a specific node with local storage. Moving to a High Availability (HA) setup (e.g., using Raft storage) would prevent data loss and downtime if the node fails or is replaced.
