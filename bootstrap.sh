...
case "$DISTRO" in
  'ubuntu' | 'debian')
    sudo apt-get update
    sudo apt-get install -y build-essential curl file git zlib1g-dev libbz2-dev liblzma-dev libssl-dev libsqlite3-dev libreadline-dev libffi-dev libncurses5-dev
    ;;
  'fedora' | 'centos' | 'rhel')
    sudo dnf groupinstall -y "Development Tools"
    sudo dnf install -y curl file git zlib-devel bzip2-devel xz-devel openssl-devel sqlite-devel readline-devel ffi-devel ncurses-devel
    ;;
  'macOS')
